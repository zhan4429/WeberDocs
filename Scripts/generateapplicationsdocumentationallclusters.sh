...
luasource="$weber/gcc/6.3.0"
generateListOfMissingFiles
generateLuaFilesIfNew

luasource="$weber/gcc/7.3.0"
generateListOfMissingFiles
generateLuaFilesIfNew
...
luasource="$weber/intel/18.0.1"
generateListOfMissingFiles
generateLuaFilesIfNew

# Generate Lua files from intel-mpi
luasource="$weber/intel-mpi/2017.1.132-7cnwoo3/intel/17.0.1"
generateListOfMissingFiles
generateLuaFilesIfNew

luasource="$weber/intel-mpi/2018.1.163-y5t3mz6/intel/18.0.1"
generateListOfMissingFiles
generateLuaFilesIfNew


# Generate Lua files from openmpi
ls "$weber/openmpi/" > openmpifolders.txt
readarray -t openmpifolders < openmpifolders.txt
rm openmpifolders.txt

for foldername in ${openmpifolders[@]}; do
    workingdirectory=$PWD
    cd $weber/openmpi/$foldername
    innerfoldername=`ls`
    cd $innerfoldername
    innermostfoldername=`ls`
    corefiles=$PWD
    cd "$workingdirectory"
    luasource="$weber/openmpi/$foldername/$innerfoldername/$innermostfoldername"
    generateListOfMissingFiles
    generateLuaFilesIfNew
done

# Generate Lua files from tools
luasource="$weber/tools"
generateListOfMissingFiles
generateLuaFilesIfNew

# Generate Lua files from Core
luasource="$weber/core"
generateListOfMissingFiles
generateLuaFilesIfNew

# Remove all files that had default.lua in their subfolder in Core
for dir in $weber/core/*/
do
...
# Remove all files that had default.lua in their subfolder in tools
for dir in $weber/tools/*/
do
    if [ -f "$dir/default*" ];
    then
        echo ""
